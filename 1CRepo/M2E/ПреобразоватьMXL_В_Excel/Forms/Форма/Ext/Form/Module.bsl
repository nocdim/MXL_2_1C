...
Процедура ПреобразоватьТЗВExcel(ТЗ, РежимПреобразования, РежимРаботы = Неопределено)
	...
	ИначеЕсли РежимПреобразования = "4" Тогда
		...
		Для Индекс = 0 По ТЗ.Колонки.Количество() - 1 Цикл
			ДополненнаяТЗ.Колонки.Добавить(ТЗ.Колонки[Индекс].Имя, ТЗ.Колонки[Индекс].ТипЗначения);
		КонецЦикла;
		ДополненнаяТЗ.Колонки.Добавить("Активность", Новый ОписаниеТипов("Строка"));
		ДополненнаяТЗ.Колонки.Добавить("СчетДт", Новый ОписаниеТипов("Строка"));
		ДополненнаяТЗ.Колонки.Добавить("СчетКт", Новый ОписаниеТипов("Строка"));
		ДополненнаяТЗ.Колонки.Добавить("ВидСубконтоДт1", Новый ОписаниеТипов("Строка"));
		ДополненнаяТЗ.Колонки.Добавить("СубконтоДт1", Новый ОписаниеТипов("Строка"));
		ДополненнаяТЗ.Колонки.Добавить("ВидСубконтоДт2", Новый ОписаниеТипов("Строка"));
		ДополненнаяТЗ.Колонки.Добавить("СубконтоДт2", Новый ОписаниеТипов("Строка"));
		ДополненнаяТЗ.Колонки.Добавить("ВидСубконтоДт3", Новый ОписаниеТипов("Строка"));
		ДополненнаяТЗ.Колонки.Добавить("СубконтоДт3", Новый ОписаниеТипов("Строка"));
		ДополненнаяТЗ.Колонки.Добавить("ВидСубконтоКт1", Новый ОписаниеТипов("Строка"));
		ДополненнаяТЗ.Колонки.Добавить("СубконтоКт1", Новый ОписаниеТипов("Строка"));
		ДополненнаяТЗ.Колонки.Добавить("ВидСубконтоКт2", Новый ОписаниеТипов("Строка"));
		ДополненнаяТЗ.Колонки.Добавить("СубконтоКт2", Новый ОписаниеТипов("Строка"));
		ДополненнаяТЗ.Колонки.Добавить("ВидСубконтоКт3", Новый ОписаниеТипов("Строка"));
		ДополненнаяТЗ.Колонки.Добавить("СубконтоКт3", Новый ОписаниеТипов("Строка"));
		Для каждого СтрТЗ Из ТЗ Цикл
			СтрДопТЗ = ДополненнаяТЗ.Добавить();
			СтрДопТЗ.Дата = СтрТЗ.Дата;
			...
			СтрДопТЗ.Содержание = СтрТЗ.Содержание;
			//СтрДопТЗ.Комментарий = СтрТЗ.Комментарий;
			...
			Если ДокБазаИнфо.ПометкаНаУдаление Тогда
				...
				Продолжить;
			КонецЕсли;
			Для каждого СтрокаТЗДвиж Из ДокБазаИнфо.Движения Цикл
				ДопСтрДопТЗ = ДополненнаяТЗ.Добавить();
				ДопСтрДопТЗ.Активность = Строка(СтрокаТЗДвиж.Активность);
				ДопСтрДопТЗ.СчетДт = Строка(СтрокаТЗДвиж.СчетДт);
				ДопСтрДопТЗ.СчетКт = Строка(СтрокаТЗДвиж.СчетКт);
				ДопСтрДопТЗ.ВидСубконтоДт1 = Строка(СтрокаТЗДвиж.ВидСубконтоДт1);
				ДопСтрДопТЗ.СубконтоДт1 = Строка(СтрокаТЗДвиж.СубконтоДт1);
				ДопСтрДопТЗ.ВидСубконтоДт2 = Строка(СтрокаТЗДвиж.ВидСубконтоДт2);
				ДопСтрДопТЗ.СубконтоДт2 = Строка(СтрокаТЗДвиж.СубконтоДт2);
				ДопСтрДопТЗ.ВидСубконтоДт3 = Строка(СтрокаТЗДвиж.ВидСубконтоДт3);
				ДопСтрДопТЗ.СубконтоДт3 = Строка(СтрокаТЗДвиж.СубконтоДт3);
				ДопСтрДопТЗ.ВидСубконтоКт1 = Строка(СтрокаТЗДвиж.ВидСубконтоКт1);
				ДопСтрДопТЗ.СубконтоКт1 = Строка(СтрокаТЗДвиж.СубконтоКт1);
				ДопСтрДопТЗ.ВидСубконтоКт2 = Строка(СтрокаТЗДвиж.ВидСубконтоКт2);
				ДопСтрДопТЗ.СубконтоКт2 = Строка(СтрокаТЗДвиж.СубконтоКт2);
				ДопСтрДопТЗ.ВидСубконтоКт3 = Строка(СтрокаТЗДвиж.ВидСубконтоКт3);
				ДопСтрДопТЗ.СубконтоКт3 = Строка(СтрокаТЗДвиж.СубконтоКт3);
			КонецЦикла; 
		КонецЦикла;
